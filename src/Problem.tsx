...
import { Problem as ProblemType } from "@/App";
...
export function Problem(props: ProblemProps): JSX.Element {
  ...
  function handleSuccessClick() {
    fetch(`${endpoint}/success/${pkey}`, {
      method: "GET"
    }).then((res) => res.json()).then((data: ProblemType) => {
  ...
  return (
    <Fragment>
      <Grid item xs={4} md={3} onContextMenu={handleContextMenu} style={{
        display: "flex",
        ...
        <ButtonGroup variant="contained">
          ...
          <Button
            variant={"outlined"}
            color={color}
            onClick={handleSuccessClick}>{success}</Button>
          <Button
            variant={"outlined"}
            color={color}
            onClick={handleFailClick}>{fail}</Button>
        </ButtonGroup>
      </Grid>
      <Menu
