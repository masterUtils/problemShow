...
const App: FC = () => {
  ...
  useEffect(() => {
    if (section && !open) {
      fetch(`${endpoint}/stats/_/${section}.json`).then(res => res.json()).then(setProblem);
    }
  }, [section, open]);
  ...
  return (
    <Container>
      <Root>
        <CssBaseline />
        <Dialog open={open} onClose={handleDialogClose} maxWidth={"md"} style={{
          minWidth: "500px"
        }}>
        ...
        <Grid container spacing={2}>
          {Object.entries(problem).sort().map(([pkey, p]) => {
